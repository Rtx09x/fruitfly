...
class ConnectomeSimulation:
    ...
    def __init__(self, cache: Path, seed: int = 7):
        ...
        self.stimulus = {"motion_left": 0.0, "motion_right": 0.0, "loom": 0.0}
        self.paused = False
        self.tick = 0
        self.hz = 0.0
    ...
    def step(self):
        ...
        with self.lock:
            ml, mr, loom = (self.stimulus[k] for k in ("motion_left", "motion_right", "loom"))
    ...
    def state(self):
        ...
        speed = min(1.0, (lm + rm) * 12.0)
        turn = max(-1.0, min(1.0, (rm - lm) * 10.0))
        ...
        return {"tick": self.tick, "step_ms": round(self.last_step_ms, 1), "sim_hz": round(self.hz, 2),
                ...
                "points": points.round(4).tolist(), "paused": self.paused, "stimulus": self.stimulus,
